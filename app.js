import LightningFS from 'https://cdn.jsdelivr.net/npm/@isomorphic-git/lightning-fs/+esm';
import * as git from 'https://cdn.jsdelivr.net/npm/isomorphic-git/+esm';
import http from 'https://cdn.jsdelivr.net/npm/isomorphic-git@1.37.2/http/web/index.js/+esm';

const STORAGE_KEY = 'browser-git-demo-settings-v1';
...
function saveSettings() {
  const { token, ...safePart } = getSettings();
  localStorage.setItem(STORAGE_KEY, JSON.stringify(safePart));
  log('已将除 token 外的设置保存到 localStorage。');
}
...
function bootstrap() {
  ...
  log('注意：这个 demo 把 token 直接用于浏览器端 HTTPS 认证，仅适合临时测试。');
}
